...
local Linux = {
    Theme = {
        ...
        TabIndicator = Color3.fromRGB(0, 120, 255) -- Cor azul pra barrinha
    }
}
...
function Linux.Create(config)
    ...
    local TabIndicator = Linux.Instance("Frame", {
        Parent = TabHolder,
        BackgroundColor3 = Linux.Theme.TabIndicator,
        Size = UDim2.new(0, 2, 0, 20), -- Tamanho ajustado: 2 pixels de largura, 20 pixels de altura
        Position = UDim2.new(0, 5, 0, 9), -- Ajustado pra ficar dentro da aba (considerando padding)
        ZIndex = 3
    })
    ...
    function LinuxLib.Tab(config)
        ...
        if config.Icon and config.Icon.Enabled then
            TabIcon = Linux.Instance("ImageLabel", {
                ...
                BackgroundTransparency = 1,
                Size = UDim2.new(0, 16, 0, 16),
                Position = UDim2.new(0, 10, 0.5, -8), -- Ajustado pra dar espaço pra barrinha
        ...
        local TabText = Linux.Instance("TextLabel", {
            Parent = TabBtn,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, (config.Icon and config.Icon.Enabled) and -31 or -15, 1, 0), -- Ajustado pra dar espaço pra barrinha
            Position = UDim2.new(0, (config.Icon and config.Icon.Enabled) and 31 or 10, 0, 0), -- Ajustado pra dar espaço pra barrinha
            Font = Enum.Font.SourceSans,
            Text = config.Name,
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            -- Animar a barrinha pra nova aba
            local tabPositionY = (tabIndex - 1) * (28 + TabLayout.Padding.Offset) + TabPadding.PaddingTop.Offset + 4 -- Ajuste pra centralizar na aba
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(TabIndicator, tweenInfo, {Position = UDim2.new(0, 5, 0, tabPositionY)}):Play()
        end)
